...
gameObject::gameObject(std::string Passed_ClassName, int passed_ID, int *passed_MouseX, int *passed_MouseY,  int X, int Y, int W, int H, float *passed_CameraX, float *passed_CameraY, CSDL_Setup* passed_csdl_setup, coordinates* passed_spawn)
{
    ...
    sprite = new CSprite(csdl_setup->GetRenderer(), FilePathSprite, Rect.x, Rect.y, Rect.w, Rect.h, CameraX, CameraY, CCollisionRectangle(hitbox.x,hitbox.y,hitbox.w,hitbox.h), csdl_setup);

    if(SpriteCropX > 0 && SpriteCropY > 0 )
        sprite->SetUpAnimation(SpriteCropX,SpriteCropY);
}
